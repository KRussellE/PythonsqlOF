...
# New button to execute SQL query and display results in the query_output_box
def execute_sql_query():
    ...
    try:
        ...
        cursor.execute("SELECT * FROM `_Orders-FinalView-v02_sync` WHERE `Tracking Number` = 'QV348657871GB'")  # Modify the table name if necessary
        ...
        if rows:
            for row in rows:
                query_output_box.insert(tk.END, f"{row}\n")
        else:
            query_output_box.insert(tk.END, "Nincs adat a táblában.")
